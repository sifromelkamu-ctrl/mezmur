// Kept in sync by hand with src/data/devotionalLines.ts on the frontend —
// same duplication pattern as morningVerses.json (see push.ts) — so the
// "Picked for You" push notifications share the exact same curated,
// on-brand copy Home's own devotional line already uses, rather than a
// separate pool to maintain.
export const DEVOTIONAL_LINES = [
