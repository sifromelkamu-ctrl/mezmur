...
**Gotcha:** cookies alone can still be inconsistent — confirmed in production that YouTube's bot-check can block lower-traffic channel content from Render's IP even with valid cookies, while the identical video/cookies work instantly from a residential IP. If imports are still failing/inconsistent after setting cookies, set `YTDLP_PROXY_URL` to a **residential/ISP** proxy (not a datacenter one — that just swaps one flagged IP for another), format `http://user:pass@host:port`. See the comment above that env var in `render.yaml`.
