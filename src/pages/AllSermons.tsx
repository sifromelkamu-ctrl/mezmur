...
import { sermonToTrack, sermonsApi, type ApiSermon } from "../lib/api";

type SortMode = "name" | "speaker";

export default function AllSermons() {
  const navigate = useNavigate();
  const { t } = useLanguage();
  const { playTrack } = usePlayer();
  const [sermons, setSermons] = useState<ApiSermon[] | null>(null);
  ...
  const sortedSermons = useMemo(() => {
    ...
    switch (sort) {
      ...
      case "speaker":
        return copy.sort((a, b) => a.speaker.localeCompare(b.speaker));
    }
  }, [sermons, sort]);

  const sortOptions: { id: SortMode; label: string }[] = [
    { id: "name", label: "Sort by Name" },
    { id: "speaker", label: "Sort by Speaker" },
  ];
  ...
  return (
    <div className="px-6 py-6">
      <div className="flex items-center justify-between mb-6">
        ...
        <h1 className="text-lg font-bold">{t("sermons")}</h1>
      ...
      {error ? (
        <div className="text-fg-muted text-sm bg-elevated/50 rounded-lg p-4 max-w-md">
          Couldn't load sermons. Check your connection and try again.
        </div>
      ) : sermons === null ? (
        ...
        <div className="text-fg-muted text-sm bg-elevated/50 rounded-lg p-4 max-w-md">No sermons yet.</div>
      ) : (
        <div className="grid grid-cols-2 gap-3">
